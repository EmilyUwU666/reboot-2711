...
async function request(release, url, method = 'GET', body) {
    const response = await fetch('http://127.0.0.1:3551' + url, {
        method, body: body === undefined ? undefined : JSON.stringify(body),
        headers: {'content-type': 'application/json', 'user-agent': `Fortnite/++Fortnite+Release-${release}-CL-1 Windows/10`},
...
async function main() {
    ...
    const releases = ['19.40', '20.40', '21.51', '22.40', '23.50', '24.40', '25.30', '26.30', '27.11', '28.30', '29.40', '30.40', '19.40'];
    for (const release of releases) {
        ...
        const match = await (await request(release, '/fortnite/api/matchmaking/session/season-test')).json();
        ...
        console.log(`Season ${season} HTTP/profile checks passed (${release}, synthetic CL 1)`);
    }
    console.log(`${checks} cross-season backend checks passed across all twelve seasons and a return to Season 19`);
}
